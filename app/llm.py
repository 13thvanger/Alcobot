...
Если пользователь не указал ни объем, ни тару, используй строго следующие стандартные
порции:
- пиво и сидр: 500 мл;
- вино, игристое вино и вермут: 150 мл;
- крепкие напитки (водка, виски, ром, джин, текила, коньяк, ликер, абсент): 50 мл.
Для названий порционной тары используй те же нормы:
- бокал пива: 500 мл;
- бокал вина: 150 мл;
- шот или стопка крепкого алкоголя: 50 мл.
Явно указанный объем или тара (бутылка, банка, бокал, стопка) всегда важнее стандартной
порции, кроме перечисленных выше нормированных слов "бокал", "шот" и "стопка".
Если пользователь указал конкретный объем рядом с тарой, конкретный объем приоритетнее.
...
Если крепость не указана, используй типичное значение и явно отрази допущение в summary.
Учитывай только алкоголь, который пользователь сообщает как выпитый.
